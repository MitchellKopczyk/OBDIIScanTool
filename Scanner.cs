...
namespace OBDIIToolKit
{
    ...
    class Scanner
    {
        ...
        //SQLite Objects 
        private static SQLiteConnection dbConnnection;
        ...
        private static FaultCode[] GetFaultMemory()
        {
            ...
            dbConnnection = new SQLiteConnection("Data Source=Diagnostic Trouble Codes;Version=3;");
            dbConnnection.Open();
            ...
            while (FaultCodeDump.Length >= 4)
            {
                ...
                if (FaultCode[0] == 'P')
                    ...
                    list.Add(new FaultCode(FaultCode, "No Description Found"));

                //Ready query to be executed
                SQLiteCommand command = new SQLiteCommand(sql, dbConnnection);
                SQLiteDataReader reader = command.ExecuteReader();

                //read query response and get the description of the fault code
                while (reader.Read())
                    list.Add(new FaultCode(FaultCode, reader["Description"].ToString()));
        ...
        static void Main(string[] args)
        {
            ...
            Console.Write("SCANNING FOR PROTOCOL...");
            if (!SetProtocol())
            {
                Console.Clear();
            ...
            Console.Clear();
            Console.Write("READING FAULT MEMORY...");
            ...
            FaultCode[] faults = GetFaultMemory();

            Console.WriteLine("+--------+---------------------------------------------------------------+");
            Console.WriteLine(": Fault  : Description                                                   :");
            Console.WriteLine("+--------+---------------------------------------------------------------+");
            foreach (FaultCode t in faults)
                Console.WriteLine(":  " + t.getCode() + " + " + t.getDescription());
            Console.WriteLine("+--------+---------------------------------------------------------------+");
        }
